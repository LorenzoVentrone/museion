...
// Main App component for the 3D customizer canvas
export const CustomizerCanvas = ({ position = [0, 0, 2.5], fov = 25 }) => {
  ...
  return (
    <Canvas shadows camera={{ position, fov }} gl={{ preserveDrawingBuffer: true }}>
      ...
      <CameraRig>
        <Backdrop />
        {/* Render either the Shirt or Hat model based on state */}
        <Center>
...
// Backdrop component: renders animated shadows under the model
function Backdrop() {
...
// CameraRig component: animates the camera and group rotation based on state
...
// Shirt component: renders the shirt model with a customizable decal (logo)
...
// Preload the shirt model and all possible logo textures for better performance
...
// Hat component: renders the hat model with a customizable decal (logo)
function Hat(props) {
  const snap = useSnapshot(state);
  const texture = useTexture(`/images/models_logo/${snap.decal}.png`);
...
// Preload the hat model and all possible logo textures for better performance
